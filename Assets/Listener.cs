...
public class Listener : MonoBehaviour
{
    ...
    public float[] RightHandData = new float[3]; // Created 2 arrays for left and right hand
    ...
    void ParseData(string data)
    {
        ...
        if (values[0] == "Left")
        { // Added to differentiate between left and right hand data
            LeftHandData[0] = float.Parse(values[1]);
            LeftHandData[1] = float.Parse(values[2]);
            LeftHandData[2] = float.Parse(values[3]);
            RightHandData[0] = float.Parse(values[5]);// Added to parse right hand data
            RightHandData[1] = float.Parse(values[6]);
            RightHandData[2] = float.Parse(values[7]);
        }
        else
        {
            RightHandData[0] = float.Parse(values[1]);// Added to parse right hand data
